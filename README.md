...
La regla de shell evita marcar comandos que DexAuditor puede demostrar como estáticos y también plantillas cuya única interpolación sea `process.pid` o `process.ppid`. Para varias superficies de alta señal, el analizador sigue ahora asignaciones locales simples dentro de la misma función/bloque: una fuente como `req.query`, `req.params`, `req.body`, `process.argv`, `event.data` o `location.*` puede propagarse a través de aliases hasta SQL, filesystem, HTTP saliente, navegación/redirects, carga de módulos, selección de ejecutables, CORS y sinks HTML. La propagación respeta la asignación visible más reciente, parámetros, shadowing de bloques, declaraciones TypeScript y código sin punto y coma; una reasignación estática o un sanitizador HTML source-visible corta la señal cuando puede demostrarse localmente. DexAuditor no intenta inferir seguridad por nombres arbitrarios ni seguir dataflow entre funciones. Las rutas que alcanzan un sink desde una fuente de menor confianza permanecen `needs_validation`; en `DEXJS004`, HTML dinámico sin una fuente demostrable se degrada a `hardening`, mientras que sanitización HTML source-visible demostrable se suprime.
